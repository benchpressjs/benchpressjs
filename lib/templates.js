...
(function(module) {
	...
	function unpad(template) {
		...
		pieces.head = pieces.template.slice(0, match.index);
		pieces.template = pieces.template.slice(match.index);


		if (pieces.head.slice(-1) === '/') {
			pieces.head = pieces.head.slice(0, pieces.head.length-2);
			pieces.template = '/' + pieces.template;
		}

		return pieces;
	}

	function repad(pieces) {
		return pieces.head + pieces.template + pieces.tail;
	}

	function parse(template, obj, namespace) {
		namespace = namespace || '';

		var pieces = unpad(template);

		for (var key in obj) {
			if (obj.hasOwnProperty(key)) {
				if (typeof obj[key] === 'undefined' || typeof obj[key] === 'function' || !pieces.template.match(key)) {
					continue;
				} else if (obj[key] === null) {
					pieces.template = replaceValue(pieces.template, namespace + key, '');
				} else if (obj[key].constructor === Array) {
					pieces.template = replaceValue(pieces.template, namespace + key + '.length', obj[key].length);
					pieces.template = parseArray(pieces.template, obj, key, namespace);
				} else if (obj[key] instanceof Object) {
					pieces.template = parseObject(pieces.template, obj, key, namespace);
					pieces.template = checkConditionals(pieces.template, key, obj[key]);
					pieces.template = parse(pieces.template, obj[key], namespace + key + '.');
				} else {
					pieces.template = parseValue(pieces.template, namespace + key, obj[key]);	
				}
			}
		}
		...
		if (namespace) {
			...
			pieces.template = checkConditionalHelpers(pieces.template, obj);
			pieces.template = cleanup(pieces.template);
		}
