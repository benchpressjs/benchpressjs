...
(function(module) {
	...
	templates.parse = function(template, block, obj, callback) {
		...
		function restoreSymbol(template) {
		...
		if (loader && callback) {
			if (!templates.cache[template]) {
				loader(template, function(loaded) {
					...
					launchCallback(loaded, obj, block, restoreSymbol);
				});
			} else {
				launchCallback(templates.cache[template], obj, block, restoreSymbol);
			}
		} else if (callback) {
			launchCallback(template, obj, block, restoreSymbol);
		} else {
