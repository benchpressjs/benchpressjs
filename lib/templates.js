...
(function(module) {
	...
	templates.parse = function(template, obj, callback) {
		...
		if (loader && callback) {
			if (!templates.cache[template]) {
				loader(template, function(loaded) {
					...
					callback(parse(loaded, obj));
				});	
			} else {
				callback(parse(templates.cache[template], obj));
			}
		} else {
			return parse(template, obj);
		}
	};
	...
	function parse(template, obj, namespace, blockInfo) {
		namespace = namespace || '';
