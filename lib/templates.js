...
(function(module) {
	...
	if (typeof self !== 'undefined') {
		self.addEventListener('message', function(e) {
		  self.postMessage({
		  	result: templates.parse(e.data.template, e.data.object),
	...
	function launchWorker(template, obj, callback) {
		...
		if (loader && callback) {
			if (!templates.cache[template]) {
				loader(template, function(loaded) {
					...
					if (worker) {
						launchWorker(loaded, obj, callback);
					} else {
				...
				if (worker) {
					launchWorker(templates.cache[template], obj, callback);
				} else {
