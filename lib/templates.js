...
(function(module) {
	...
	templates.parse = function(template, obj, block, callback) {
		...
		for (var fn in functions) {
			if (functions.hasOwnProperty(fn)) {
				...
				if (helpers[method]) {
					...
					result = replace(result, new RegExp(func, 'gi'), callMethod(helpers[method], parameters).replace('$&', '$$$&'));
				}
			}
		}
	...
	function parseArray(template, array, key, namespace) {
		...
		while (block = template.match(regex)) {
			...
			do {
				...
				result = parseFunctions(block, result.replace(/\$+/g, '$$$'), {
