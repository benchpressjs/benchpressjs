...
(function() {
	...
	templates.parse = function (data, template) {
		...
		return (function parse(data, namespace, template, blockInfo) {
			...
			function parseFunctions(block, result, data, d, i, numblocks) {
				...
				for (var fn in functions) {
					if (functions.hasOwnProperty(fn)) {
						...
						if (templates.helpers[method]) {
							result = result.replace(new RegExp(func, 'gi'), templates.helpers[method].apply(this, [data[d][i], i, numblocks]));
						}
					}
				}
