...
(function(module) {
	...
	function makeBlockRegex(block) {
		return new RegExp('(<!--[\\s]*BEGIN ' + block + '[\\s]*-->[\\s]?)|(<!--[\\s]*END ' + block + '[\\s]*-->)', 'g');
	}
		...
		if (matches !== null) {
			for (var i = 0, ii = matches.length; i < ii; i++) {
				var statement = makeStatementRegex(key),
					...
					match = matches[i].replace(statement, '').replace(/[\s|\S]<!-- IF[\s\S]*ENDIF[\s\S]*-->/gi, '<!-- NESTED -->'),
	...
	function parseFunctions(block, result, parameters) {
		var functions = block.match(/\s*?{function.*?}\s*?/gi, '');
	...
	function parseArray(template, array, key, namespace, bind) {
		...
		while (block = template.match(regex)) {
			...
			do {
				parsedBlock = parse(block, array[key][iterator], bind, namespace, {iterator: iterator, total: numblocks}).replace(/\r\n?|\n$/, '');
