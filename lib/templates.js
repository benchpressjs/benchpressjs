...
(function(module) {
	...
	function parseArray(template, array, key, namespace) {
		template = checkConditionals(template, namespace + 'length', array[key].length);

		var regex = makeRegex(key), block, result;
		...
		while ((block = template.match(regex)) !== null) {
			...
			result = parseArrayBlock(template, block, array[key], namespace);
