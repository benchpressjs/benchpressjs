...
(function(module) {
	...
	templates.parse = function(template, block, obj, callback) {
		...
		if (loader && callback) {
			...
			return getBlockThenParse(block, template, obj);
		}
	};
	...
	function launchCallback(template, obj, block, callback) {
		if (worker) {
			...
			callback(getBlockThenParse(block, template, obj));
		}
	}

	function getBlockThenParse(block, template, obj) {
		block = !block ? template : templates.getBlock(template, block);
		template = parseTemplate(block, obj);
	...
	function parseTemplate(template, obj, namespace) {
		namespace = namespace || '';

		var arrays = {}, objects = {}, kv = {};
		for (var key in obj) {
			if (obj.hasOwnProperty(key)) {
				if (obj[key] === null || typeof obj[key] === 'undefined' || typeof obj[key] === 'function' || !template.match(key)) {
					continue;
		...
		template = parse(template, kv, namespace);
		template = parse(template, objects, namespace);
		template = parse(template, arrays, namespace);
	...
	function parseArrayBlock(template, block, array, namespace, isObject) {
		...
		for (var iterator in array) {
			if (array.hasOwnProperty(iterator)) {
				...
				result += parseTemplate(block, array[iterator], namespace);
				if (!isObject) {
	...
	function parse(template, obj, namespace) {
		for (var key in obj) {
			if (obj.hasOwnProperty(key)) {
				if (namespace && (obj[key] === null || typeof obj[key] === 'undefined' || typeof obj[key] === 'function' || !template.match(key))) {
					continue;
				} else if (obj[key].constructor === Array) {
					...
					template = parseTemplate(template, obj[key], namespace + key + '.');
				} else {
	...
	function cleanup(template) {
		...
		return parseUndefined ? parseTemplate(template, undefinedKeys, '') : template;
	}
