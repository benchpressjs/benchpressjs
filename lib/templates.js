...
(function(module) {
	...
	function parseTemplate(block, template, obj) {
		...
		return parseFunctions(template, template, {
			data: obj
		});
	}
	...
	function parseFunctions(block, result, parameters) {
		...
		for (var i=0, ii=functions.length; i<ii; ++i) {
			var func = functions[i],
				method = functions[i].split('.').pop().split('}').shift();
	...
	function parseArray(template, array, key, namespace) {
		...
		while (block = template.match(regex)) {
			...
			do {
				...
				result = result.replace(regexes.parseArray, '');
			...
			template = replace(template, regex, result.replace(regexes.parseArray, ''));
		}
