...
(function(module) {
	...
	function parseArray(template, array, key, namespace) {
		...
		while ((block = template.match(regex)) !== null) {
			...
			result = parseArrayBlock(template, block, array[key], namespace + key + '.');
	...
	function parseArrayBlock(template, block, array, namespace) {
		...
		for (var key in obj) {
			if (obj.hasOwnProperty(key)) {
				if (typeof obj[key] === 'undefined' || typeof obj[key] === 'function') {
					...
					var subspace = namespace.split('.');
					do {
						template = parseValue(template, subspace.join('.') + key, obj[key]);	
					} while (subspace.shift());
				}
			}
		}
