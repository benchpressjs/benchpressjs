...
(function(module) {
	...
	function cleanup(template, obj) {
		...
		for (var undefinedKey in undefinedKeys) {
			if (undefinedKeys.hasOwnProperty(undefinedKey)) {
				
				undefinedKey = regexes.getUndefinedKeys.exec(undefinedKeys[undefinedKey]);
				
				if (undefinedKey) {					
					undefinedKey = undefinedKey[1]

					if (template.match(new RegExp('<!-- ENDIF[\\s!]*' + undefinedKey + ' -->'))) {
						keysToReparse[undefinedKey] = false;
					}
				}
			}
		}
