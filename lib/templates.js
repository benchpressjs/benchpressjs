...
(function(module) {
	...
	templates.parse = function(template, block, obj, callback) {
		...
		function restoreSymbols(template) {
			callback(template.replace(/<\!-- \$ -->/g, '$'));
		}
		...
		if (loader && callback) {
			if (!templates.cache[template]) {
				loader(template, function(loaded) {
					...
					launchCallback(loaded, obj, block, restoreSymbols);
				});
			} else {
				launchCallback(templates.cache[template], obj, block, restoreSymbols);
			}
		} else if (callback) {
			launchCallback(template, obj, block, restoreSymbols);
		} else {
			return parseTemplate(block, template, obj).replace(/<\!-- \$ -->/g, '$');
		}
	};
