...
(function(module) {
		...
		if (matches !== null) {
			for (var i = 0, ii = matches.length; i < ii; i++) {
				...
				if (conditionalBlock[1]) {
					...
					template = replace(template, matches[i], replace(conditionalBlock[value ? 0 : 1], regexes.checkConditional, ''));
				} else {
					// regular if statement
					template = replace(template, matches[i], value ? replace(match, regexes.checkConditional, '') : '');
				}
	...
	function parseArray(template, array, key, namespace) {
		...
		while (block = template.match(regex)) {
			...
			do {
				...
				result = result.replace(regexes.removeTabspace, '');

				result = parseFunctions(block, result, array[key][iterator]);
	...
	function cleanup(template) {
		template = template.replace(regexes.cleanupEmptyLoops, '');

		var undefinedKey,
			undefinedKeys = {},
			cleanup;

		while (undefinedKey = regexes.getUndefinedKeys.exec(template)) {
			undefinedKeys[undefinedKey[1]] = false;
			cleanup = true;
		}

		return cleanup ? parse(template, undefinedKeys, '') : template;
	}
