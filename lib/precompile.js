...
const compile = (() => {
  try {
    try {
      ...
      return require(`../../rust/benchpress-rs/pre-built/${process.platform}_${process.versions.modules}`).compile;
    }
  } catch (e) {
    // eslint-disable-next-line no-console
    console.warn('[benchpressjs] Unable to build or find a suitable native compiler, falling back to JS version');
...
function precompile(source, options, callback) {
  ...
  const promise = Promise.try(() => {
    ...
    const compiled = (opts.unsafe ? compileFallback : compile)(source, opts);
